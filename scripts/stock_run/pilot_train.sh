#!/bin/sh
# Walk-forward pilot training -- identical to the committed baseline_train.sh
# (same EXAMM args, same binary, deliberately NO save-verification hook so the
# pilot runs under the SAME conditions as the baseline; corrupted saves are
# filtered afterwards by sweep_verify.py, exactly as the baseline was), but
# with the data and output locations parametrized:
#
#   DATA     (default datasets/walkforward/pilot_2023)
#   BASE_OUT (default test_output/pilot_2023)
#
# usage: DATA=... BASE_OUT=... sh scripts/stock_run/pilot_train.sh AKAM ATO ...

REPO=$(cd "$(dirname "$0")/../.." && pwd)
BIN=$REPO/build/mpi/examm_mpi
DATA=${DATA:-$REPO/datasets/walkforward/pilot_2023}
BASE_OUT=${BASE_OUT:-$REPO/test_output/pilot_2023}
...
if [ ! -x "$BIN" ]; then
    echo "ERROR: $BIN not found -- build examm first" >&2
    exit 1
fi
LAUNCHER=${MPI_LAUNCH%% *}
...
if [ $# -gt 0 ]; then
    ...
    TICKERS=$(ls "$DATA" | sed -E 's/_(train|val|test)\.csv$//' | sort -u)
fi
